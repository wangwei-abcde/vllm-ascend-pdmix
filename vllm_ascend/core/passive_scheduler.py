...
class PassiveScheduler:
    ...
    def schedule(
        ...
        When ``target_batch_type`` is set (cross-DP coordination active), the
        EEP/EED state machine is consulted first: if it naturally agrees with
    ...
    def _intended_batch_type(self) -> BatchType | None:
        ...
        Normal (non-coordinated) ``schedule()`` calls this internally to
        check whether the EEP/EED state machine agrees with the coordinated
        target before deciding to run the normal path or force-dispatch.
    ...
    def _force_schedule_target(
        ...
        coordination winner is selected and the local state machine either
        has nothing to offer or disagrees on the category.

        The dummy batch has ``total_num_scheduled_tokens == 0`` and
        ``is_pd_dummy == True`` so the cloud worker runs ``_dummy_run``:
        still participates in EP all-toall but skips real hidden-state
        computation.
        """
